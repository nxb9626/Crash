...
async def bot():
    ...
    move = mini_maxi(fen=fen_string, weights={'max_depth':2})
...
def mini_maxi(fen,weights):
    ...
    """
    next_boards = generate_positions(fen)
...
def min_max(weighted_move, weights, depth=0)-> WeightedMove:
    ...
    next_boards = generate_positions(str(weighted_move.fen))
...
def generate_positions(fen) -> WeightedMove:
    """
    ...
    for move in current_board.legal_moves:
        ...
        next_boards.append(WeightedMove(fen=str(current_board.board_fen()),move=move))
        current_board.pop()
